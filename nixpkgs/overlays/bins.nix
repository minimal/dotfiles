...
self: super:
let
  # Version + per-platform tarball data for br/bv below. Generated data:
  # refresh with bin/bump-beads.bb (or `just bump-beads`), do not hand-edit
  # unless you know what you are doing (nix-prefetch-url --unpack hashes).
  beads-sources = builtins.fromJSON (builtins.readFile ./beads-sources.json);
  # beads_rust (br) and beads_viewer (bv) - agent-first issue tracker and its
  # companion viewer. Version + per-platform release-tarball hashes are data,
  # not code: they live in ./beads-sources.json, regenerated by
  # ../../../bin/bump-beads.bb (or `just bump-beads` from the repo root).
  #
  # br uses the statically-linked musl build on Linux: the gnu prebuilds require
  # glibc >= 2.38 and fail on older distros (e.g. Ubuntu 22.04 / glibc 2.35).
  # bv is a statically linked Go binary, so no glibc concerns on any distro.
  mkBeadsPkg = { pkg, description, homepage }:
    with super;
    let
      entry = beads-sources."${pkg}";
      source = entry.sources."${stdenv.hostPlatform.system}";
    in
      stdenv.mkDerivation rec {
        ...
        src = fetchzip {
          ...
          name = pkg;
          stripRoot = false; # tarball has the binary + docs at top level
        };
        phases = ["installPhase"];
        installPhase = ''
          mkdir -p $out/bin
          cp -p $src/${pkg} $out/bin/${pkg}
        '';
        meta = with lib; {
          inherit description homepage;
          # MIT with OpenAI/Anthropic rider (custom variant, not plain MIT)
          license = licenses.mit;
          platforms = builtins.attrNames entry.sources;
        };
      };
